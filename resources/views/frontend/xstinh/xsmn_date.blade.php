...
@section('content')
    <div class="col-l" style="height: auto !important;">
        @if(!empty($xsmn))
            ...
            $xsDau = getDau($xsmnLoto, substr($xsmn->gdb, strlen($xsmn->gdb) - 2, 2));

            $xsDuoi = getDuoi($xsmnLoto, substr($xsmn->gdb, strlen($xsmn->gdb) - 2, 2));

            ?>
            <div class="box" id='kqngay_{{getNgayID($xsmn->date)}}'>
                <div class="tit-mien clearfix">
                    <h2>XS{{strtoupper($province->short_name)}} - XS
                        đài {{$province->name}} {{getNgay($xsmn->date)}}</h2>

                    <div>
                        <a class="sub-title" href="{{route('xsmn')}}" title="XSMN">XSMN</a>
                        » <a class="sub-title" href="{{route('xstinh.tinh',$province->slug)}}"
                             title="XS{{strtoupper($province->short_name)}}">XS{{strtoupper($province->short_name)}}</a> {{getThu($xsmn->day)}}
                        » <a class="sub-title"
                             href="{{route('xstinh.date',[$province->short_name,getNgayLink($xsmn->date)])}}"
                             title="XS{{strtoupper($province->short_name)}} {{getNgay($xsmn->date)}}">XS{{strtoupper($province->short_name)}} {{getNgay($xsmn->date)}}</a>
                    </div>
                </div>
                ...
                <div class="see-more">
                    ...
                    <ul class="list-html-link two-column">
                        ...
                        <li>Xem bảng kết quả <a
                           ...
                           <p> <strong> XS{{strtoupper($province->short_name)}} {{$province->name}}  {{getNgay($date)}} - Kết quả xổ số {{$province->name}} hôm nay nhanh nhất tại xosobadai.net Xổ số {{getNgay($date)}}</strong>
                            </p>
                            </li>Trực tiếp {{strtoupper($province->short_name)}} {{getNgay($date)}} ngày hôm nay nhanh, chính xác nhất.</li>
        
                            XS{{strtoupper($province->short_name)}} {{$province->name}}  {{getNgay($date)}}  - Kết quả xổ số XS{{strtoupper($province->short_name)}} {{$province->name}}      <strong> Xổ số XS{{strtoupper($province->short_name)}} {{$province->name}}  </strong> </li> trực tiếp lúc 17  giờ 15 phút. Xổ số hôm nay XS{{strtoupper($province->short_name)}} {{$province->name}}    hàng tuần được mở thưởng bởi công ty xổ số kiến thiết của đài hôm nay  . Kết quả xổ số XS{{strtoupper($province->short_name)}} {{$province->name}}    </li> bắt đầu quay từ giải tám cho đến giải nhất và cuối cùng là giải đặc biệt.
                            
                            Kết quả XS{{strtoupper($province->short_name)}} {{$province->name}} {{getNgay($date)}}  - Xổ số miền XS{{strtoupper($province->short_name)}} {{$province->name}}  
                            ...
                            <p>Kết quả xổ số XS{{strtoupper($province->short_name)}} {{$province->name}} {{getNgay($date)}}  được công bố trên các trang web xosobadai.net đề hoặc trên các kênh truyền hình vào mỗi ngày. </p>
