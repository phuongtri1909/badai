<div class="the-article-content">
    <h2 class="the-article-summary">Dự đoán XSMN {{$date}} - Soi cầu dự đoán xổ số miền Nam ngày {{$date}} do cao thủ chốt số đưa ra siêu chuẩn,
        miễn phí ❤️ Dự đoán lô tô, giải đặc biệt MN ngày {{str_replace('/','-',$date)}}</h2>
    ...
    <p>Hãy cùng chuyên gia {{request()->getHost()}} <a
                href="{{route('dudoan.xsmn.date',getNgayLink($dateYmd))}}">dự
    ...
    <p>Để kết quả dự đoán được chính xác Xổ Số Miền Nam XSMN Hôm Nay. Bạn nên tham khảo lại kết quả KQXS Xổ Số Miền Nam XSMN các kỳ trước để có cơ  sở so sánh và đưa ra quyết định chọn con số phù hợp của XSMN, có cơ hội trúng giải cao nhất.</p>
    ...
    @foreach ($province_mn as $pro)
        <div class="pascal">
            ...
            <div class="pascal-body"><p>Giải đặc biệt: <span
                            class="red conso_dudoan">{{$pascal[$pro->short_name]['gdb']}}</span></p>
    ...
    <p>BÍ KÍP giúp {{request()->getHost()}} cầu XSMN {{$date}} và các ngày khác đều cho kết quả dự đoán chính xác
    ...
    <p>Các bạn có thể thử vận may tìm ra con số may mắn của mình bằng cách tham gia <a target="_blank" href="{{route('quay_thu.mn')}}">Quay
            thử XSMN</a></p>
    ...
    <p>Để có kết quả cầu xsmn {{$date}} các chuyên gia giỏi nhất của {{request()->getHost()}} phải tổng hợp dữ liệu
