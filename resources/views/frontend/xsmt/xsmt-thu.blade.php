...
@section('content')
    <div class="col-l">
        ...
        {{-- </div> --}}
        <ul class="tab-panel">
            <li><a href="{{ route('xsmt') }}">miền Trung</a>
            </li>
            <li @if ($day == 2) class="active" @endif><a href="{{ route('xsmt.thu_2') }}"
                    title="XSMT Thứ 2">Thứ
                    2</a></li>
            <li @if ($day == 3) class="active" @endif><a href="{{ route('xsmt.thu_3') }}"
                    title="XSMT Thứ 3">Thứ
                    3</a></li>
            <li @if ($day == 4) class="active" @endif><a href="{{ route('xsmt.thu_4') }}"
                    title="XSMT Thứ 4">Thứ
                    4</a></li>
            <li @if ($day == 5) class="active" @endif><a href="{{ route('xsmt.thu_5') }}"
                    title="XSMT Thứ 5">Thứ
                    5</a></li>
            <li @if ($day == 6) class="active" @endif><a href="{{ route('xsmt.thu_6') }}"
                    title="XSMT Thứ 6">Thứ
                    6</a></li>
            <li @if ($day == 7) class="active" @endif><a href="{{ route('xsmt.thu_7') }}"
                    title="XSMT Thứ 7">Thứ
                    7</a></li>
            <li @if ($day == 8) class="active" @endif><a href="{{ route('xsmt.cn') }}"
                    title="XSMT Chủ nhật">CN</a></li>
        </ul>
        @if (!$kqToDay && $day_now == $day)
            <div class="block" id="mt_kqngay_{{ date('dmY', time()) }}" style="display: none;">
        ...
        @include('frontend.xsmt.xsmt_include_thu')
        ...
        <div class="box-content">
            ...
            @foreach ($province_mt as $pro)
                <span>
                    <li>
                        <a style="text-decoration: underline" href="{{ route('xstinh.tinh', $pro->slug) }}"
                            title="Xổ số {{ $pro->name }}">
                            <span>
                                Xổ số {{ $pro->name }}
                            </span>
                        </a>
            ...
            <h2><strong>Cơ cấu giải thưởng <a style="text-decoration: underline" title="xổ số miền Trung"
                        href="{{ url()->full() }}">xổ số miền Trung</a></strong>
            </h2>
        ...
        $(document).ready(function() {
            ...
            $('#xem_them').click(function(event) {
                ...
                if (skip <= last_page) {
                    ...
                    $.post("{{ route('xsmt.xem_them_theo_thu') }}", {
