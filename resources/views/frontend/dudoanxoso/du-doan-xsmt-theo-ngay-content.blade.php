<div class="the-article-content">
    <h2 class="the-article-summary">Soi cầu dự đoán xổ số miền Trung ngày {{$date}} do các cao thủ chốt số đưa ra siêu chuẩn, miễn phí ❤️ Dự đoán lô tô, giải đặc biệt miền Trung ngày {{$date}}</h2>
    ...
    <p>Hãy cùng chuyên gia {{request()->getHost()}} <a
                href="{{route('dudoan.xsmt.date',getNgayLink($dateYmd))}}">dự
    ...
    <p>BÍ KÍP giúp {{request()->getHost()}} cầu XSMT {{$date}} và các ngày khác đều cho kết quả dự đoán chính xác
    ...
    <p>Các bạn có thể thử vận may tìm ra con số may mắn của mình bằng cách tham gia <a target="_blank" href="{{route('quay_thu.mt')}}">Quay
            thử XSMT</a></p>
    ...
    <p>Để có kết quả cầu xsmt {{$date}} các chuyên gia giỏi nhất của {{request()->getHost()}} phải tổng hợp dữ liệu
